...
/**
 * @author tajo 读写分离的数据源，可配置一个write和多个read-only的数据源，并提供对read-only数据源做负载均衡的接口 借鉴了LazyConnectionDataSourceProxy的connection
 *         Proxy实现方式 没有去检查所有注册的数据源的conneciton的isolation和commit的默认属性。如有需要，在初始化的过程中检测 该类仍然为一个抽象类，需要实现load balance的策略
 */
public abstract class AbstractRWRoutingDataSourceProxy
    ...
    implements InitializingBean
{
     ...
     */
    private class RWConnectionInvocationHandler
        implements InvocationHandler
    {
            ...
            throws SQLException
        {
            ...
            if ( this.target == null )
            {
                ...
                this.target =
                    ( this.username != null ) ? determineTargetDataSource( key ).getConnection( this.username, this.password )
