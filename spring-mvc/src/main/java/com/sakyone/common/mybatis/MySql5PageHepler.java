...
public class MySql5PageHepler
{
    ...
    public static String getCountString( String querySelect )
    {
        ...
        int appearNumber = fromAppearNumber( querySelect );
        String select = querySelect.substring( 0, formIndex );

        // 如果SELECT 中包含 DISTINCT 只能在外层包含COUNT
        if ( select.toLowerCase().indexOf( "select distinct" ) != -1 || querySelect.toLowerCase().indexOf( "group by" ) != -1 || appearNumber > 1 )
        {
    ...
    public static int fromAppearNumber(String querySelect) {
        int count = 0;
        String regex = "\\s+FROM\\s+";
        Pattern p = Pattern.compile( regex, Pattern.CASE_INSENSITIVE );
        Matcher m = p.matcher(querySelect);
        while (m.find()) {
            count++;
        }
        return count;
